import './App.css'
const Footer= ()=>{
    return(
        <div className='footer'>
             <div className="social-media">
             <a href='https://wa.me/8270224796?text=Hello' target='_blank'><i className="fa-brands fa-whatsapp fa-2xl  icon-btn" style={{color:'white'}}></i></a>
             <a href='https://www.linkedin.com/in/aathish11' target='_blank'><i className="fa-brands fa-linkedin fa-2xl" style={{color:'white'}}></i></a>
             <a href='https://www.instagram.com/itzz.aathi/' target='_blank'><i className="fa-brands fa-instagram fa-2xl" style={{color:'white'}}></i></a>
             <a href='https://github.com/aathi-aathi' target='_blank'><i className="fa-brands fa-github fa-2xl" style={{color:'white'}}></i></a>
       
       
        
        </div>
        <p>Aathish Arul © 2024</p>
        </div>
       
    )
}
export default Footer;